...
Mergedle (mergedle.com) is an unofficial Merge Tactics fan site with 3 active + 2 coming-soon Wordle-style daily puzzle games. No accounts, no server — all game state lives in localStorage. Daily tacticians are seeded deterministically by UTC date + game slug.
...
│   ├── classic/            ← daily: guess tactician from stat clues
│   ├── pixel/              ← daily: guess tactician from pixelated card image
│   ├── skin/               ← daily: guess tactician from skin image
│   ├── about-us/
│   ├── privacy-policy/
│   └── terms-of-service/
├── components/
...
│   ├── tactician-stats.ts      ← Classic + Pixel game tactician pool + stats
│   ├── traits.ts               ← trait data (display clues + future modes)
│   ├── skin-cards.ts           ← Skin game pool
│   └── streakManager.ts
└── public/
    ├── Tacticians/    ← tactician card art WebP
...
- **Daily seed = `dayKey + "_" + gameSlug`** — changing this string changes every user's daily tactician. Be careful.
...
- **Tactician image paths**: `/Tacticians/{Title-Case-Dashes}.webp` (e.g. `archer_queen` → `Archer-Queen.webp`).
...
## Two Core Data Types
1. **Tactician** — `lib/tactician-stats.ts` — the guessable object in all modes
...
2. Fill tactician data in `lib/tactician-stats.ts` (all fields required)
3. Fill trait data in `lib/traits.ts`
4. Fill skin pool in `lib/skin-cards.ts` (only tacticians where hasSkins: true)
